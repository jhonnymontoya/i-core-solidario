				...
				<tbody>
					@foreach ($ahorros as $ahorro)
						<tr data-modalidad-id="{{ $ahorro->modalidad_ahorro_id }}" data-modalidad-nombre="{{ $ahorro->codigo }} - {{ $ahorro->nombre }}" data-cuota="{{ round($ahorro->cuota, 0) }}" data-saldo="{{ round($ahorro->saldo, 0) }}">
							...
							<td>
								<a class="btn btn-outline-info btn-sm" data-toggle="modal" data-target="#mAhorros"><i class="fa fa-plus"></i></a>
							</td>
						</tr>
					@endforeach
				</tbody>
	...
	function actualizarAhorros() {
		...
		data.ahorros.forEach(function(ahorro){
			...
			if(!ahorro.visible) {
				...
				detalle.append($("<td>").html("<a class='btn btn-outline-danger btn-sm aLimpiar'><i class='fa fa-trash'></i></a>"));
				ahorro.visible = true;
