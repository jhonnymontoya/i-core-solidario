				...
				<div class="card-body">
					...
					@if($socio)
						...
						<ul class="nav nav-tabs" role="tablist">
							...
							<li role="presentation">
								<a href="#beneficiarios" aria-controls="beneficiarios" role="tab" data-toggle="tab">Beneficiarios</a>
							</li>
							<li role="presentation">
						...
						<div class="tab-content">
											...
											<table class="table table-hover">
												...
												<tbody>
													@foreach($creditos as $credito)
														<tr>
															<td>
																<a href="{{ route('consulta.creditos', $credito->id) }}">{{ $credito->numero_obligacion }}</a>
															</td>
															<td>{{ $credito->modalidadCredito->nombre }}</td>
															<td>{{ $credito->fecha_desembolso }}</td>
															<td class="text-right">${{ number_format($credito->valor_credito, 0) }}</td>
											...
											<table class="table table-hover recaudos">
												...
												<tbody>
													...
													@foreach($recaudos as $recaudo)
														...
														?>
														<tr>
															<td><a href="{{ route('consulta.recaudos', [$recaudo->controlProceso->id]) }}">{{ $recaudo->controlProceso->calendarioRecaudo->numero_periodo }}.{{ $recaudo->controlProceso->calendarioRecaudo->fecha_recaudo }}</a></td>
							...
							<div role="tabpanel" class="tab-pane fade" id="beneficiarios">
								<br>
								<div class="row">
									<div class="col-md-11 table-responsive">
										@if($socio->beneficiarios->count())
											<table class="table table-hover">
												<thead>
													<tr>
												...
												<tbody>
													@foreach($socio->beneficiarios as $beneficiario)
														<tr>
															...
															<td class="text-right">{{ $beneficiario->porcentaje_beneficio }}%</td>
														</tr>
													@endforeach
												</tbody>
											</table>
										@else
											<br>
											<label>No hay registros de beneficiarios para mostrar</label>
											<br><br>
										@endif
									</div>
								</div>
...
<div class="modal fade" id="mct" tabindex="-1" role="dialog" aria-labelledby="mLabel">
	{!! Form::open(["route" => "consulta.documentacion", "method" => "get", "target" => "_blank"]) !!}
