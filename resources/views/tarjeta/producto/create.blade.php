					...
					<div class="row">
						...
						<div class="col-md-3">
							<div class="form-group">
								...
								{!! Form::text('convenio', null, ['class' => [$valid, 'form-control'], 'autocomplete' => 'off', 'placeholder' => 'Número de convenio']) !!}
					...
					<div class="row">
						<div class="col-md-3">
							<?php
								...
								if(!$credito and !$ahorro and !$vista) {
									$credito = true;
								}
							...
							<div class="form-group">
								...
								<div>
									@php
										...
										switch (old('tipo_pago_cuota_manejo')) {
											case 'ANTICIPADO':
...
<script type="text/javascript">
	$(function(){
		...
		@if(!empty(old('cuenta_compensacion_cuif_id')))
			$.ajax({url: '{{ url('cuentaContable/getCuentaConParametros') }}', dataType: 'json', data: {id: {{ old('cuenta_compensacion_cuif_id') }} }}).done(function(data){
		...
		@if(!empty(old('ingreso_comision_cuif_id')))
			$.ajax({url: '{{ url('cuentaContable/getCuentaConParametros') }}', dataType: 'json', data: {id: {{ old('ingreso_comision_cuif_id') }} }}).done(function(data){
				if(data.total_count == 1)
				{
					element = data.items[0];
		...
		@if(!empty(old('egreso_comision_cuif_id')))
			$.ajax({url: '{{ url('cuentaContable/getCuentaConParametros') }}', dataType: 'json', data: {id: {{ old('egreso_comision_cuif_id') }} }}).done(function(data){
				if(data.total_count == 1)
				{
					element = data.items[0];
		...
		@if(!empty(old('cuota_manejo_cuif_id')))
			$.ajax({url: '{{ url('cuentaContable/getCuentaConParametros') }}', dataType: 'json', data: {id: {{ old('cuota_manejo_cuif_id') }} }}).done(function(data){
				if(data.total_count == 1)
				{
					element = data.items[0];
