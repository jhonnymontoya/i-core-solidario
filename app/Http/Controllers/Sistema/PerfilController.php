...
class PerfilController extends Controller
{
	...
	public function index(Request $request) {
		...
		foreach($entidades as $entidad)$listaEntidades[$entidad->id] = $entidad->terceroEntidad->nombre_corto;
		$perfiles = Perfil::with(['entidad', 'menus'])->entidadId($request->entidad)->search($request->name)->activo($request->estado)->paginate();
		return view('sistema.perfil.index')->withPerfiles($perfiles)->withEntidades($listaEntidades);
	}

	public function create() {
		$entidades = Entidad::activa()->get()->sortBy('terceroEntidad.razon_social')->pluck('terceroEntidad.razon_social', 'id');
		$menus = Menu::listarTodos();
		return view('sistema.perfil.create')->withMenuList($menus)->withEntidades($entidades);
	}
	...
	public function store(CreatePerfilRequest $request) {
		...
		try {
			DB::transaction(function() use($perfil, $request){
				$perfil = Entidad::find($request->entidad_id)->perfiles()->create($request->all());
				$perfil->menus()->attach(Menu::find($request->menus));
			});
		}
		catch(Exception $e) {
	...
	public static function routes() {
		...
		Route::get('perfil/{obj}/edit', 'Sistema\PerfilController@edit')->name('perfilEdit');
